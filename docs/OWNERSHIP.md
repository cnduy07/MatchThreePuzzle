...
| Shared Scene Flow & UI Shell | `Assets/Scripts/Flow/SceneFlow.cs`, `SceneBootstrapper.cs`, `SceneNames.cs`, `Assets/Scripts/UI/RuntimeUiShell.cs`, `RuntimeUiFactory.cs`, `RuntimeSettingsState.cs`, `SafeAreaRoot.cs`, `Assets/Scripts/MessageWindow.cs`, `ScreenFader.cs`, `RectXformMove.cs`, `Singleton.cs` | Very High | Used by every scene and every major UI flow. Shared runtime shell and scene flow changes can affect menu, level select, gameplay HUD, overlays, save/progression display, and reusable scene loading. Treat these shared files as exclusive. |
| Menu Scene Builder | `Assets/Scripts/Flow/MenuSceneUiBuilder.cs` | Medium | File-specific menu layout work after the builder split. Do not edit shared scene flow, shared UI factory, scenes, prefabs, or Shared APIs from this domain. |
| Level Map Scene Builder | `Assets/Scripts/Flow/LevelMapSceneUiBuilder.cs` | Medium | File-specific level-map layout work after the builder split. Do not edit shared scene flow, shared UI factory, level data, scenes, prefabs, or Shared APIs from this domain. |
| Game Scene HUD Builder | `Assets/Scripts/UI/GameSceneHudBuilder.cs` | Medium | File-specific gameplay HUD, threat preview, and booster-bar layout work after the builder split. Do not edit `RuntimeUiShell.cs`, game rules, scenes, prefabs, or Shared APIs from this domain. |
...
- For screen-focused parallel work, use the separate `Menu Scene Builder`, `Level Map Scene Builder`, and `Game Scene HUD Builder` domains. Shared files such as `SceneFlow.cs`, `SceneBootstrapper.cs`, `RuntimeUiFactory.cs`, and `RuntimeUiShell.cs` still require exclusive coordination through `Shared Scene Flow & UI Shell`.
