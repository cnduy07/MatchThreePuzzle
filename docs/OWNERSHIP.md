...
| Scene Flow & UI Shell | `Assets/Scripts/Flow/SceneFlow.cs`, `SceneBootstrapper.cs`, `SceneNames.cs`, `MenuSceneUiBuilder.cs`, `LevelMapSceneUiBuilder.cs`, `Assets/Scripts/UI/RuntimeUiShell.cs`, `GameSceneHudBuilder.cs`, `RuntimeUiFactory.cs`, `RuntimeSettingsState.cs`, `SafeAreaRoot.cs`, `Assets/Scripts/MessageWindow.cs`, `ScreenFader.cs`, `RectXformMove.cs`, `Singleton.cs` | Very High | Used by every scene and every major UI flow. Shared runtime shell and scene flow changes can affect menu, level select, gameplay HUD, overlays, save/progression display, and reusable scene loading. After the builder split, screen-focused sessions should claim only the specific builder file they will edit whenever possible. |
...
- For screen-focused parallel work, prefer file-specific claims inside Scene Flow & UI Shell: `MenuSceneUiBuilder.cs` for menu work, `LevelMapSceneUiBuilder.cs` for level-map work, and `GameSceneHudBuilder.cs` for gameplay HUD work. Shared files such as `SceneFlow.cs`, `SceneBootstrapper.cs`, `RuntimeUiFactory.cs`, and `RuntimeUiShell.cs` still require exclusive coordination.
